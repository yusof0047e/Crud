...
def Accept(e):
    ...
    if not username or not password:
        messagebox.showerror("هشدار", "لطفا ایمیل و گذرواژه را بدرستی وارد کنید")
        print("پسورد یا نام کاربری نامعتبر ثبت شده.")
    else:
        print("روی ثبت کردن کلیک شد.")
        messagebox.showinfo("افرین","با موفقیت ثبت نام شد و شما به صفحه بعد ارسال خواهید شد")
        form.destroy()
...
def login(e):
    form.destroy()
    python_file_path = "signup_persian.py"
    subprocess.Popen(["python", python_file_path], shell=True)
def subserver(e):
    form.destroy()
...
#lbl
lbl_user = Label(text="ایمیل: ", font="IRHoma", fg="black", width=10)
lbl_user.place(x=180, y=245)
lbl_pass = Label(text="گذرواژه: ", font="IRHoma", fg="black", width=10)
lbl_pass.place(x=180, y=285)
#link
lbl_passyad = Label(text="آیا گذرواژه را فراموش کرده اید؟", font="IRHoma", fg="#79AC78", cursor="hand2")
lbl_passyad.place(x=250, y=330)
lbl_passyad.bind("<Button-1>", lambda event: passyad())
#btn_Language
btn_language=Button(text="English", font="IRHoma")
...
#btn_signup
btn_signup = Button(text="ثبت نام", width=5, font="IRHoma")
